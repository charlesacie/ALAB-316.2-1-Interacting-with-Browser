/*In order to demonstrate how JavaScript and DOM events can enhance form validation, 
we will walk through a small example together. Fork the sandbox below, 
and code along as we continue with this example.
First, we will create a validateEmail function that handles two simple rules:

An email address must contain at least an @ sign and a dot (.).
The @ sign must not be the first character of the email address, 
and the last dot must be at least one character after the @ sign. 
Now, we are going to refactor some of that code and incorporate validateEmail into a larger function that validates all of our form components.
 Remove the evt argument and all evt.returnValue references, and unbind validateEmail from the form element.
Let's build out the skeleton of the validate() function:
